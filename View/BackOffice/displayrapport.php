...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Ensure the 'rapport_id' and 'action' are set
    if (isset($_POST['rapport_id']) && isset($_POST['action'])) {
        $rapportId = $_POST['rapport_id'];
        $action = $_POST['action'];

        // Instantiate the Rapportc controller
        $rapportObj = new Rapportc();

        if ($action === 'accept') {
            // Update the report status to 'accepted'
            $rapportObj->updateRapportStatus($rapportId, 'accepted');
        } elseif ($action === 'reject') {
            // Delete the report
            $rapportObj->deleteRapport($rapportId);
        }

        // Redirect to the same page to refresh the report list
...
if (!empty($rapports)) {
    ...
    foreach ($rapports as $rapport) {
        ...
        echo '<form method="post" action="">
                <input type="hidden" name="rapport_id" value="' . htmlspecialchars($rapport['id_rapport']) . '">
    ...
    echo '<div class="alert alert-warning" role="alert">Aucun rapport trouvé.</div>';
}
?>
