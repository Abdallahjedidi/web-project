...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($titre)) {
      ...
      if (str_word_count($titre) < 1) $errors[] = "Le titre doit contenir au moins un mot.";
      if (!preg_match('/[a-zA-Z\xc0-\xff]/u', $titre)) $errors[] = "Le titre doit contenir des lettres.";
  }
  ...
  if (empty($description)) {
      ...
      if (str_word_count($description) < 2) $errors[] = "La description doit contenir au moins deux mots.";
      if (!preg_match('/[a-zA-Z\xc0-\xff]/u', $description)) $errors[] = "La description doit contenir des lettres.";
  }

  if (empty($emplacement)) $errors[] = "L'emplacement est requis.";
  if (empty($date_signalement)) $errors[] = "La date du signalement est requise.";
          ...
          <div class="form_container">
            ...
            <form action="signalement_ajout.php" method="POST">
              ...
              <div class="input-group mb-2">
                <input type="text" name="titre" id="titre" class="form-control" placeholder="Titre du signalement*" value="<?= htmlspecialchars($_POST['titre'] ?? '') ?>">
                <div class="input-group-append">
                  <button class="btn btn-outline-secondary" type="button" onclick="startVoice('titre')"><i class="fa fa-microphone"></i></button>
                </div>
              </div>

              <!-- Description avec micro -->
              <div class="input-group mb-2">
                <textarea name="description" id="description" class="form-control" placeholder="Description*" rows="4"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                <div class="input-group-append">
                  <button class="btn btn-outline-secondary" type="button" onclick="startVoice('description')"><i class="fa fa-microphone"></i></button>
                </div>
              </div>
...
function startVoice(fieldId) {
  try {
    ...
    alert("Ce navigateur ne prend pas en charge la reconnaissance vocale.");
  }
}
</script>
